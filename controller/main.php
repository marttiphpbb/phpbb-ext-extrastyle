...
use marttiphpbb\extrastyle\core\timespan;
...
class main
{
	...
	*/
	public function monthview(int $year, int $month):Response
	{
		...
		$days_prefill = $month_start_weekday - $this->config['extrastyle_first_weekday'];
		...
		$this->event_container->set_timespan($timespan)
			->fetch()
			->create_event_rows($this->config['extrastyle_min_rows'])
			->arrange();
		...
		$this->template->assign_vars([
			...
			'YEAR'			=> $year,
			'U_YEAR'		=> $this->helper->route('marttiphpbb_extrastyle_yearview_controller', [
				'year' => $year]),
		]);
